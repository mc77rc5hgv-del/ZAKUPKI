...
export function parseFilter(input:string): TenderFilter {
  ...
  const aliases:Record<string,string>={"запрос":"query","q":"query","ключи":"includeKeywords","включить":"includeKeywords","исключить":"excludeKeywords","минцена":"minPrice","максцена":"maxPrice","заказчик":"customer","регион":"location","место":"location","район":"districts","районы":"districts","территория":"districts","статус":"status","окпд":"okpd2","дней":"days","документы":"requireDocuments","сорт":"sort"};
  for(const segment of input.split(";").map(x=>x.trim()).filter(Boolean)){
    ...
    if(key==="includeKeywords"||key==="excludeKeywords"||key==="okpd2"||key==="districts") (filter as any)[key]=list(value);
