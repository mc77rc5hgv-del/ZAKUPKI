...
assertBotConfig(); await loadStore(); await loadDeviceStore();
function describeError(e: unknown): string {
  const message = e instanceof Error ? e.message : String(e);
  if (/AGENT_OFFLINE/.test(message)) return "Локальный мост не подключён. Откройте Mini App → Подключение → «Подключить компьютер».";
...
bot.command("connection",async ctx=>showConnection(ctx));
bot.command("devices",async ctx=>showDevices(ctx));
bot.command("disconnect",async ctx=>disconnectCommand(ctx));
bot.command("security",async ctx=>ctx.reply(securityText));
