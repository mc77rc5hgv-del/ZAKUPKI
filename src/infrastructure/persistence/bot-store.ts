...
export type PipelineItem = { url: string; title: string; stage: PipelineStage; note?: string; deadlineAt?: string; updatedAt: string };
...
export async function setPipeline(id:number,url:string,title:string,stage:PipelineStage,note?:string,deadlineAt?:string){user(id).pipeline[url]={url,title,stage,note,deadlineAt,updatedAt:new Date().toISOString()};await save();return user(id).pipeline[url];}
